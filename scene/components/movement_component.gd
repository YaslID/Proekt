...
@export var acceleration: float = 5 # чем больше это тем быстрее мобы реагировать будут
...
func accelerate_to_direction(direction: Vector2):
	...
	current_velocity = current_velocity.lerp(final_velocity, 1- exp(-acceleration * get_process_delta_time())) # шаг независимый от скорости кадра и фреймов)
