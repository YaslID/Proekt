...
func _process(delta):
	...
	velocity = movement_component.accelerate_to_direction(direction) #плавный переход
	move_and_slide()
...
func check_if_damaged():
	if enemies_colliding == 0 || !grace_period.is_stopped(): #чтоб игрок урон не получал постоянно
		return
	health_component.take_damage(enemy_damage) #если спорикасается то урон наносим тут а выше не соприкасается написано
	grace_period.start()
...
func _on_grace_period_timeout(): #подсоединили сигнал чтоб бага не было с уроном
	check_if_damaged()
