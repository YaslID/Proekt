...
func _ready():
	player = get_tree().get_first_node_in_group("player") #сделали доступ к плееру
	base_speed = movement_component.max_speed #100
...
func burst_shot():
	for i in range(3):
		await get_tree().create_timer(0.1 * i).timeout #задержка дольше будет
		single_shot()
...
func _on_attack_timer_timeout():
	if not player: return #чтоб ошибок не было
	if attack_range:
		match current_phase:
			Phase.Phase_1:
				single_shot()
