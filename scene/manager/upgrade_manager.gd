...
@export var experience_manager: ExperienceManager #привязка
...
var current_upgrades = {} #словарь типо массив но более функциональный
...
func pick_upgrades():
	...
	for i in 3: #делаем цикл для карточек
		if upgrade_pool.upgrades.size() == chosen_upgrades.size(): # если размер массива = 0
			break
		var chosen_upgrade = upgrade_pool.pick_upgrade(chosen_upgrades) 
		chosen_upgrades.append(chosen_upgrade) #добавл новый элемент
	
	
	return chosen_upgrades

func on_upgrade_selected(upgrade: AbilityUpgrade): #поймет какую карту выбрал и какую улучшить надо
